...
conn =  psycopg2.connect(host="localhost", dbname="postgres", user="postgres", password="9704507171", port="5432")
	...
	def parseAdisIMU(self, sensor_id, bag_file_id, bag_file, topic, output_file_name):
		...
		try:
			...
			self.db.bulk_insert(table='adis_imu', fields=['sensors_id', 'bag_files_id', 'timestamp', 'seconds', 'nanoseconds', 'time',
														  'x_acceleration', 'y_acceleration', 'z_acceleration', 'x_angular_velocity', 'y_angular_velocity', 'z_angular_velocity'], file_name=output_file_name)
...
#####################################################
	...
	def setSensor(self,id,product_name):
		...
		return self.db.insert(table='sensors', fields=['id', 'product_name'],
							  ...
							  conflict_on=['id'])
		try:
			# Check if sensor already exists and retrieve its ID
			self.cursor.execute("SELECT id FROM sensors WHERE product_name = %s", (product_name,))
			result = self.cursor.fetchone()
			if result:
				return result[0]
			else:
				# Insert new sensor and return its ID
				self.cursor.execute("INSERT INTO sensors (product_name) VALUES (%s) RETURNING id", (product_name,))
